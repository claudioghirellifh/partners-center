...
<div class="grid gap-5 md:grid-cols-2">
    ...
    <div>
        <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Desconto anual (%)</label>
        <input type="number" min="0" max="100" name="annual_discount_percentage" value="{{ old('annual_discount_percentage', $plan->annual_discount_percentage ?? '') }}" class="mt-2 w-full rounded-lg border border-slate-300 bg-white/80 px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-[#F27327] focus:ring-2 focus:ring-[#F27327]/30 dark:border-slate-700 dark:bg-slate-950/70 dark:text-white">
        @error('annual_discount_percentage')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
    </div>
</div>
