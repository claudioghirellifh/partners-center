...
@section('header', 'Editar versão / aviso')

@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <p class="text-sm text-slate-500 dark:text-slate-400">Atualize a versão, notas ou avisos exibidos no painel dos admins.</p>
            <p class="text-xs text-slate-400 dark:text-slate-500">Versão atual: {{ $releaseNote->version }}</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white/85 p-8 shadow-lg shadow-slate-200/60 backdrop-blur dark:border-slate-800 dark:bg-slate-900/60 dark:shadow-slate-950/30">
        <form action="{{ route('adminroot.release-notes.update', $releaseNote) }}" method="POST" class="space-y-6">
            @include('adminroot.release-notes._form', ['submitLabel' => 'Salvar alterações'])
        </form>
    </div>
@endsection
