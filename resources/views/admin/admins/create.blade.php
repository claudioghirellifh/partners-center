...
@section('content')
    <p class="text-sm text-slate-500 dark:text-slate-400">
        Preencha os dados para convidar um novo administrador. Ele receberá um e-mail com as credenciais de acesso.
    </p>

    <form action="{{ route('admin.admins.store', ['company' => $company]) }}" method="POST" class="mt-6 grid gap-6 md:max-w-3xl">
        @csrf

        <div class="grid gap-5 md:grid-cols-2">
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Nome completo</label>
                <input type="text" name="name" value="{{ old('name') }}" required class="mt-2 w-full rounded-lg border border-slate-300 bg-white/80 px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-[color:var(--brand)] focus:ring-2 focus:ring-[color:var(--brand)]/30 dark:border-slate-700 dark:bg-slate-950/70 dark:text-white">
        ...
        <div class="md:w-1/3">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Status inicial</label>
            <select name="is_active" class="mt-2 w-full rounded-lg border border-slate-300 bg-white/80 px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-[color:var(--brand)] focus:ring-2 focus:ring-[color:var(--brand)]/30 dark:border-slate-700 dark:bg-slate-950/70 dark:text-white">
                <option value="1" @selected(old('is_active', '1')==='1')>Ativo</option>
                <option value="0" @selected(old('is_active')==='0')>Suspenso</option>
            </select>
